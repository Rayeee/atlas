...
public class BeanHelper {
     ...
     * @author lonaking
     */
    public static <B, D> List<D> list2list(List<B> one, Class<D> cla) {
        List<D> listD = new ArrayList<D>();
        for (B b : one) {
            D d = BeanHelper.bean2bean(b, cla);
            listD.add(d);
        }
        return listD;
    }
     ...
     * @author lonaking
     */
    public static <B, D> D bean2bean(B bean, Class<D> cla) {
        try {
            D dto = cla.newInstance();
            BeanUtils.copyProperties(bean, dto);
            return dto;
    ...
    public static <B, D> D bean2bean(B bean, Class<D> cla, boolean camelUpperTrans) {
        try {
            if (camelUpperTrans) {
                D dto = cla.newInstance();
    ...
    public static <B, D> List<D> list2list(List<B> one, Class<D> cla, boolean camelUpperTrans) {
        try {
            if (camelUpperTrans) {
                List<D> listD = new ArrayList<D>();
                for (B b : one) {
                    D d = BeanHelper.bean2bean(b, cla, true);
                    listD.add(d);
                }
                return listD;
            } else {
                return list2list(one, cla);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
